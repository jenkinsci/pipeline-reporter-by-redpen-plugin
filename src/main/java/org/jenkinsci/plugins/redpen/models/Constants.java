...
public class Constants {
    ...
    public static final ImmutableList<TestFrameWork> TEST_FRAME_WORKS = ImmutableList.of(
            TestFrameWork.builder().displayName(WEB_DRIVER_IO_DISPLAY_NAME).path("/reports/E2E-report.pdf").value(WEB_DRIVER_IO).build(),
            TestFrameWork.builder().displayName(JUNIT_DISPLAY_NAME).path("/logs").value(JUNIT).build(),
            TestFrameWork.builder().displayName(JACOCO_DISPLAY_NAME).path("/target/jacoco.exec").value(JACOCO).build(),
            TestFrameWork.builder().displayName(NUNIT_DISPLAY_NAME).path("/logs").value(NUNIT).build(),
            TestFrameWork.builder().displayName(JEST_DISPLAY_NAME).path("/jest-stare/jest-results.json").value(JEST).build());
